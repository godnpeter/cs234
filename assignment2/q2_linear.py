...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        self.s = tf.placeholder(tf.uint8, [None, state_shape[0],state_shape[1] , state_shape[2]*4], name = 'state1')
        self.a = tf.placeholder(tf.int32, [None], name ='action')
        self.r = tf.placeholder(tf.float32, [None], name = 'reward')
        self.sp = tf.placeholder(tf.uint8, [None, state_shape[0],state_shape[1] , state_shape[2]*4], name = 'state2')
        self.done_mask = tf.placeholder(bool, [None], name = 'done')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope , reuse):
            ...
            out= tf.layers.dense(inputs=inputs, units=num_actions, bias_initializer=tf.zeros_initializer())
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_sa = tf.add(self.r, tf.multiply(done, tf.multiply(gamma, tf.reduce_max(target_q, axis=1))))
    ...
    def add_optimizer_op(self, scope):
        ...
        grads_1 = self.optimizer.compute_gradients(self.loss,update_vars)
 
       
        if self.config.grad_clip is True:
            grads = [(tf.clip_by_norm(grad, self.config.clip_val), var) for grad, var in grads_1]
           
        self.train_op = self.optimizer.apply_gradients(grads)
        self.grad_norm = tf.global_norm([item[0] for item in grads_1]) 
