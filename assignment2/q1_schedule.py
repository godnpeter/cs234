...
class LinearSchedule(object):
    ...
    def update(self, t):
        ...
        one_step_inter = round(float(self.eps_begin - self.eps_end) / float(self.nsteps), 5)
        print(one_step_inter)
        while(t>0):
            if(self.epsilon> self.eps_end and one_step_inter <= self.epsilon - self.eps_end):
                self.epsilon -= one_step_inter
            t -= 1
            self.epsilon=round(self.epsilon, 10)
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action):
        ...
        prob = random.uniform(0, self.eps_begin)
...
def your_test():
    ...
    """
    env = EnvTest((5, 5, 1))
    exp_strat = LinearSchedule(1, 0, 10)
    exp_strat.update(5)
    exp_strat.update(15)
